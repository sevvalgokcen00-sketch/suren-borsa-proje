...
export default function TekliflerPage() {
  ...
  const [bidModal, setBidModal] = useState({
    isOpen: false,
    mode: "yeni" as "yeni" | "karsi" | "guncelle",
    targetId: null as number | null,
    title: "10mm S235JR Levha Sac Kesim Artığı",
    company: "Döngü Metal San. A.Ş.",
    price: "24000",
    amount: "12500",
    paymentType: "pesin" as "pesin" | "vadeli" | "dbs",
    incoterm: "exw" as "exw" | "ddp",
    note: "",
  });
    ...
    <div className="min-h-screen bg-[#f8fafc] font-sans flex flex-col lg:flex-row text-slate-800">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="bg-white border-b border-slate-200 px-4 sm:px-6 py-3.5 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center justify-end gap-4 text-xs">
            <button className="relative text-base p-2 bg-slate-100/80 rounded-xl hover:bg-slate-200/60 transition shrink-0">
              🔔{" "}
            ...
            <div className="border-t sm:border-t-0 sm:border-l border-slate-200 pt-3 sm:pt-0 sm:pl-4 flex items-center gap-3">
              {isLoggedIn ? (
                <div className="flex items-center gap-2 cursor-pointer">
                  <div className="w-8 h-8 rounded-full bg-[#123873] text-white font-bold flex items-center justify-center text-xs shrink-0">
                    AY
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                  <Link
                    ...
                    className="hover:opacity-90 text-white font-bold px-3 sm:px-4 py-2 rounded-xl transition shadow-sm text-center"
                  >
                    Giriş Yap
                  </Link>
                  <Link
        ...
        {/* İÇERİK */}
        <main className="w-full px-4 sm:px-6 md:px-8 py-6 space-y-6 flex-1 overflow-y-auto">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-2">
            ...
            <button
              ...
              style={{ backgroundColor: "#123873" }}
              className="hover:opacity-90 text-white font-bold px-5 py-2.5 rounded-xl text-xs shadow-sm transition flex items-center justify-center gap-1.5 shrink-0"
            >
          ...
          {/* TAB SEÇİMİ */}
          <div className="flex border-b border-slate-200 text-xs font-bold gap-4 sm:gap-6 px-2 overflow-x-auto pb-1">
            <button
                  ...
                  <table className="w-full text-left border-collapse text-xs min-w-[750px]">
                    <thead>
                      <tr className="border-b border-slate-100 text-slate-400 font-bold pb-3 uppercase text-[10px] tracking-wider">
                        ...
                        <th className="pb-3 px-3 text-center">Durum</th>
                    ...
                    <tbody className="divide-y divide-slate-100 font-medium">
                      {filteredGelen.map((item) => (
                        <tr key={item.id} className="hover:bg-slate-50/70 transition">
                          ...
                          <td className="py-4 px-3 font-mono font-bold text-slate-600 text-[11px] whitespace-nowrap">
                            ⏱ {item.expiresIn || "24s"}
                          </td>
                    ...
                    <tbody className="divide-y divide-slate-100 font-medium">
                      {filteredVerilen.map((item: any) => (
                        <tr key={item.id} className="hover:bg-slate-50/70 transition">
                          ...
                          <td className="py-4 px-3 text-slate-600 font-semibold whitespace-nowrap">🏢 {item.buyerCompanyName ?? "Bilinmiyor"}</td>
                          <td className="py-4 px-3 font-black text-[#123873] text-sm whitespace-nowrap">
                          ...
                          <td className="py-4 px-3 text-slate-500 text-xs max-w-xs">
          ...
          {!loading && !error && activeTab === "onaylanan" && (
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-6 space-y-4 w-full">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-3 px-2 gap-2">
                <div>
                ...
                <span className="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-3 py-1 rounded-lg border border-emerald-100 self-start sm:self-auto">
                  ✓ ISO 14064 Uyumlu
                </span>
              </div>
      ...
      {bidModal.isOpen && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-4xl w-full shadow-2xl border border-slate-200 overflow-hidden max-h-[92vh] flex flex-col my-auto">
            
            <div className="px-6 sm:px-8 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-[#123873]/10 text-[#123873] flex items-center justify-center text-lg font-black shrink-0">
                  🤝
                </div>
                <div>
              ...
              <button
                ...
                className="w-8 h-8 rounded-full bg-slate-200/60 hover:bg-slate-300/60 text-slate-600 font-bold text-sm flex items-center justify-center transition shrink-0"
              >
                ✕
              </button>
            </div>

            <form onSubmit={handleSaveBidModal} className="p-6 sm:p-8 overflow-y-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-7 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border-2 border-slate-200 focus:border-[#123873] focus:bg-white text-slate-900 font-black text-xl sm:text-2xl p-4 rounded-2xl outline-none transition"
                  />
              ...
              <div className="lg:col-span-5 flex flex-col justify-between bg-[#123873] text-white p-6 rounded-2xl shadow-lg space-y-6">
                ...
                <div className="space-y-5">
                  <div className="border-b border-white/10 pb-4">
                    ...
                    <h4 className="text-base font-bold mt-1 text-white truncate">
                  ...
                  <div className="bg-white/10 p-4 rounded-xl border border-white/10 flex justify-between items-baseline gap-2">
      ...
      {checkoutModal.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-slate-100 space-y-6 relative overflow-hidden my-auto">
            
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <div>
                <h3 className="font-bold text-slate-900 text-sm sm:text-base">DöngüBorsa Tescilli İşlem Onay Protokolü</h3>
              </div>
              <button 
                onClick={() => setCheckoutModal({ ...checkoutModal, isOpen: false })}
                className="text-slate-400 hover:text-slate-700 font-bold text-lg px-2 shrink-0"
              >
                ✕
              </button>
            </div>

            <div className="flex items-center justify-between px-2 text-[11px] sm:text-xs font-bold text-slate-500 gap-1">
            ...
            <div className="space-y-4 text-xs text-slate-600">
              {checkoutModal.step === 1 && (
                <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-200">
                  ...
                  <label className="flex items-start gap-2 pt-2 cursor-pointer font-medium text-slate-800">
                    <input type="checkbox" defaultChecked className="rounded border-slate-300 text-[#123873] focus:ring-[#123873] mt-0.5 shrink-0" />
              ...
              {checkoutModal.step === 2 && (
                <div className="space-y-3 bg-slate-50 p-4 rounded-2xl border border-slate-200">
                  ...
                  <div className="p-3 bg-white rounded-xl border border-slate-200 space-y-1">
                    ...
                    <p className="font-mono text-[11px] break-all" style={{ color: "#123873" }}>Alıcı Firma IBAN: TR00 1234 5678 9012 3456 7890 12</p>
      ...
      {activeNoteModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 border border-slate-200 my-auto">
